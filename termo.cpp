...
void getPalavra(char palavra[])
{
  ...
  do
  {
    std::cin.getline(palavra, 6);
    limparBuffer();

    if (!(palavraValida = eValidaPalavra(palavra)))
      std::cout << "A palavra deve ter exatamente 5 caracteres alfabéticos (minúsculos). Letras a mais serão ignoradas.\n";

  } while (!palavraValida);
}

bool eValidaPalavra(char palavra[])
{
  for (int i = 0; i < 5; i++)
  {
    ...
    converterParaCaixaBaixa(palavra[i]);
  }
  return true;
}
